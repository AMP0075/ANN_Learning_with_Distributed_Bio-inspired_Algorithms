...
class ffaAnn():
    ...
    def __init__(self, dimensions=(8, 5),
                 ...
                 alpha=0.2, alpha_damp=0.99, delta=0.05, exponent=2, m=5,
    ...
    def main(self):
        ...
        while (iterations < self.n_iterations):  # Maximum Iteration Count = 100
            ...
            # Step 3: Brightness
            population = self.parallel_updatePosition(population)
...
start_time = time.time()
a = ffaAnn(initialPopSize=100, m=10, input_values=input_val, output_values_expected=output_val)
...
Pop_size_vs_fitness = []
Pop_size_vs_time = []
for initPop in range(100,1000,50):
    start_time = time.time()
    dimns = [7, 8, 5, 10, 2, 2, 6, 1] # dimns = [random.randint(0,11) for i in range(8)] #no of layers = 10
    a = ffaAnn(initialPopSize=initPop, m = n_threads, input_values=input_val , output_values_expected=output_val, dimensions=dimns)
    temp_fit = a.main()
    end_time = time.time()
