...
class psoAnn():
    ...
    def updateVelocityPosition(self, population, velocity, start):
        ...
        for i in range(len(velocity)):
            for d in range(len(velocity[i])):
                velocity[i][d] = self.w * velocity[i][d] + \
                                 self.c1 * r1 * (self.pBest[start+i][d] - population[i][d]) + \
                                 self.c2 * r2 * (self.gBest[d] - population[i][d])

                population[i][d] += velocity[i][d]
...
start_time = time.time()
a = psoAnn(initialPopSize=100, m=10, input_values=input_val, output_values_expected=output_val)
...
for initPop in range(100,1000,50):
    start_time = time.time()
    dimns = [7, 8, 5, 10, 2, 2, 6, 1]
    a = psoAnn(initialPopSize=initPop, m = n_threads, input_values=input_val , output_values_expected=output_val, dimensions=dimns)
    temp_fit = a.main()
    end_time = time.time()
