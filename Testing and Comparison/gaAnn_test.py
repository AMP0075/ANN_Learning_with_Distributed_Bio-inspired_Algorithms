...
end_time = time.time()
print("Time for inputing data : ", end_time-start_time)

start_time = time.time()
a = gaAnn(initialPopSize=100, m = 10, bestCount = 10, input_values=input_val , output_values_expected=output_val)
...
for initPop in range(100,1000,50):
    start_time = time.time()
    dimms = [7, 8, 5, 10, 2, 2, 6, 1]
    a = gaAnn(initialPopSize=initPop, m = n_threads, bestCount = n_best, input_values=input_val , output_values_expected=output_val, dimensions=dimns)
    temp_fit = a.main()
    end_time = time.time()
    print(end_time-start_time)
