...
class InputData():
    ...
    def __init__(self, fileName="../../ANN/diabetes"):
        ...
        for j in range(len(data)):
            if(y[j]==[0]):
                output_values_expected.append([0,1])
            else:
                output_values_expected.append([1,0])
...
class ffaAnn():
    ...
    def __init__(self, dimensions=(8, 5),
                 initialPopSize=10, iterations=10, gamma=0.001, beta_base=2.6,
                 alpha=0.8, alpha_damp=0.9, delta=0.95, exponent=2, m=5,
    ...
    def main(self):
        ...
        while (iterations < self.n_iterations):  # Maximum Iteration Count = 100
            ...
            self.fitness = self.Fitness(population)[:]
