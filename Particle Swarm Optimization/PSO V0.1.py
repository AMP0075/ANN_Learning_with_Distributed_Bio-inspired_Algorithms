...
class psoAnn():
    ...
    def updateVelocityPosition(self, population, velocity):
        r1, r2 = np.random.rand(2)
        for i in range(len(velocity)):
            for d in range(len(velocity[i])):
                velocity[i][d] = self.w * velocity[i][d] + \
                                 self.c1 * r1 * (self.pBest[i][d] - population[i][d]) + \
    ...
    def swarmBestUpdate(self, population_x, fitness_x, fitness_g):
        for i in range(len(population_x)):
            if(fitness_x[i] > self.fitness[i]):
                ...
                self.fitness = self.Fitness(self.pBest)
...
a = psoAnn(fileName = "../ANN/iris", iterations = 10)
a.main()